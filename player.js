(function () {
    ...
    PlayerBase.prototype = {
                    ...
                    if (isReay == "1") {
                        ...
                        me.retryPlayTimer = setInterval(function () {
                            ...
                            if (_isReady == "1") {
                                ...
                                me.log("lastVideoError#" + _lastError + ", end video, reset")
                                me.videos[me.nextIndex].removeAttribute("lastVideoBroken");
                                me.resetVideo();
                                me.hideLoading();
                                clearInterval(me.retryPlayTimer);
                            }
                        ...
                        if (video.oAjax.status == 200 || video.oAjax.status == 202) {
                            ...
                            if (retryCount < me.options.segErrorRetryNum) {
                                ...
                                if (id >= (filesInfo.length - 1)) {
                                    var secondLastIsReady = "0";//当播放到最后一片出错时，倒数第二片是否是正确的，等于1时，上一片是正确的
                                    if (filesInfo.length > 1) {
                                        secondLastIsReady = me.videos[filesInfo.length - 2].getAttribute("ready");
                                    }
                                    me.throwErrorInfo({
                                        type: "lastVideoLoadingError",
                                        code: "1004",
                                        msg: "视频最后一片出错，放弃请求",
                                        secondLastIsReady: secondLastIsReady
                                    });
                                    if (secondLastIsReady == '1' && me.nextIndex < me.filesInfo.length) {
                                        me.videos[me.nextIndex] && me.videos[me.nextIndex].setAttribute("lastVideoBroken", "1");
                                    }
                                    else {
                                        me.resetVideo();
                                        me.hideLoading();
        ...
        resetVideo: function () {
            ...
            this.isEnded = true;
            this.seekTime = 0;
            this.options.onEndedCallBack && this.options.onEndedCallBack();
        },
